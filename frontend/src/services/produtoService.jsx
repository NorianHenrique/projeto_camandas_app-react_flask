...
export const checkNomeExists = async (nome) => {
    if (!nome) {
        console.error("Nome não fornecido para verificação");
        throw new Error("Nome é obrigatório para verificação");
    }
    
    try {
        const response = await axios.get(`${PROXY_URL}nome`, { 
            params: { nome: nome } 
        });
        return response.data;
    } catch (error) {
